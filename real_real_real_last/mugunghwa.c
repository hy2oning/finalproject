...
void mugu_manual(key_t key);
void mugu_random_players();
void mugu_tail(int i, int nx, int ny);
int check_passed();
void clear_phrase();
...
int p_row[PLAYER_MAX], p_col[PLAYER_MAX], period[PLAYER_MAX];  // 각 플레이어 위치, 이동 주기
// 통과한 player
...
void mugu_manual(key_t key) {
	...
	// 다른 플레이어와 겹치는 경우 움직임을 처리하지 않는다.
	if (!placable(n_row, n_col)) {
		return;
	}

	mugu_tail(0, n_row, n_col);

	// 움직임이 있었는지 저장한다
	// 영희가 보는 동안 움직였는지 확인하기 위해 사용한다
	if (yh_watching) {
		moved_player[0] = true;
	}
}
...
void mugu_random_players() {
	...
	for (int player_index = 1; player_index < n_player; player_index++) {
		...
		if (tick % period[player_index] == 0) {
			...
			mugu_tail(player_index, n_row, n_col);
...
void clear_phrase() {
	goto_cell(N_ROW, 0);
	// 한글 한 글자 길이 2 + 공백 1, 그러므로 한 글자당 3
	for (int i = 0; i < phrase_len * 3; i++) {
		printf(" ");
	}
}

// end 위치까지 phrase를 표시한다
void print_phrase(int end) {
	goto_cell(N_ROW, 0);
	for (int i = 0; i <= end; i++) {
		// 한글 한 글자는 아래와 같이 출력
				...
				if (exists_eliminated_player) {
					...
					for (int i = 0; i < PLAYER_MAX; i++)
					{
						if (eliminated_player[i]) {
							...
							// 탈락 표시
							player[i] = false;
							player2[i].is_alive = player[i];
							// 탈락한 player는 맵에 표시하지 않는다
							clear_player(i);
							// 살아남은 player의 수를 줄인다
							n_alive--;
						}
